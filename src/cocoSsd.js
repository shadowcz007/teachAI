...
export function getIdByClassName(t){
    if(!t)return null;
...
export async function init(){
    let localPath='indexeddb://cocoSsd';
    try {
        //先从本地缓存加载
    ...
    if(!model){
        //预热
        var zeroTensor = tf.zeros([ 1, 1, 1, 3 ]);
...
export async function run(imgTensor=null){
    if(imgTensor.shape.length==4){
        let shape=imgTensor.shape;
        shape.shift();
        imgTensor=imgTensor.reshape(shape);
        imgTensor.strides=[shape[0],shape[1]];
    };
    imgTensor=tf.cast(imgTensor,"int32");
    const predictions = await model.detect(imgTensor,20,0.3);
    return predictions;
}
